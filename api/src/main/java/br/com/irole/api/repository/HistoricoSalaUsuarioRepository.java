...
public interface HistoricoSalaUsuarioRepository extends JpaRepository<HistoricoSalaUsuario, Long>{
	...
	@Query(value = "select count(h.id) as contador, p.id as perfil_id, p.foto as foto, p.nome as nome, p.id_usuario_fk as usuario from historico_sala_usuario h left join perfil p on h.perfil_id = p.id where DATE(h.data_hora_entrada) = :date and h.data_hora_saida is not null GROUP BY h.perfil_id order by count(h.id) DESC", nativeQuery = true)
	List<Ranking> rankingDia(@Param("date") Date date);
	
	@Query(value = "select count(h.id) as contador, p.id as perfil_id, p.foto as foto, p.nome as nome, p.id_usuario_fk as usuario from historico_sala_usuario h left join perfil p on h.perfil_id = p.id  where h.data_hora_saida is not null and MONTH(h.data_hora_entrada) = MONTH(:date) and YEAR(h.data_hora_entrada) = YEAR(:date) GROUP BY h.perfil_id order by count(h.id) DESC", nativeQuery=true)
	List<Ranking> rankingMes(@Param("date") Date date);
