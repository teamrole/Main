...
public interface SalaRepository extends JpaRepository<Sala, Long>{
	...
	@Query(value="SELECT P.quantidade, I.valor, P.id AS pedido_id FROM pedido_sala AS PS, pedido_perfil AS PP, pedido AS P, item AS I, perfil AS PF WHERE PF.id = PP.perfil_id AND I.id = P.item_id AND P.id = PS.pedido_id AND PP.pedido_id = PS.pedido_id AND PS.sala_id = ?1 AND PP.perfil_id = ?2", nativeQuery = true)
	List<TotalPedido> pedidosSalaPorUsuario(Long id,Long idP);
